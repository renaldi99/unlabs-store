...
const modalWrapperClassNames = "fixed inset-0 bg-black opacity-35 z-30";
for (let index = 0; index < modalTriggers.length; index++) {
    ...
    e.addEventListener("click", function() {
        ...
        addClass(modalWrapper, "fixed inset-0 flex items-center justify-center w-100 min-h-screen z-30");

        addClass(modalOverlay, modalWrapperClassNames);
        const modalContent = document.createElement("div");
        modalContent.innerHTML = e.attributes["data-content"].value; 
        addClass(modalContent, "bg-white p-4 z-40");
